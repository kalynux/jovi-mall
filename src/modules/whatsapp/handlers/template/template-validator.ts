    ...
    validate(message: TemplateMessage): void {
        ...
        if (!message.name || message.name.trim().length === 0) {
            throw createAppError(
                ...
                { messageType: 'template', validationErrors: [{ field: 'name', message: 'Template name is required' }] }
            );
        }
        ...
        if (!message.language || message.language.trim().length === 0) {
            throw createAppError(
                ...
                400,
                'Invalid payload for message type: template',
                { messageType: 'template', validationErrors: [{ field: 'language', message: 'Template language is required' }] }
            );
        }
    ...
    private validateComponents(components: TemplateComponent[]): void {
        if (!Array.isArray(components)) {
            throw createAppError(
                ...
                400,
                'Invalid payload for message type: template',
                { messageType: 'template', validationErrors: [{ field: 'components', message: 'Components must be an array' }] }
            );
        }
        ...
        for (const component of components) {
            ...
            if (!component.type) {
                throw createAppError(
                    ...
                    400,
                    'Invalid payload for message type: template',
                    { messageType: 'template', validationErrors: [{ field: 'components.type', message: 'Component type is required' }] }
                );
            }
            ...
            if (!validTypes.includes(component.type)) {
                throw createAppError(
                    ...
                    400,
                    'Invalid payload for message type: template',
                    { messageType: 'template', validationErrors: [{ field: 'components.type', message: `Invalid component type: ${component.type}` }] }
                );
            }
            ...
            if (!component.parameters || !Array.isArray(component.parameters)) {
                throw createAppError(
                    ...
                    400,
                    'Invalid payload for message type: template',
                    { messageType: 'template', validationErrors: [{ field: 'components.parameters', message: 'Component parameters must be an array' }] }
                );
            }
    ...
    private validateParameter(param: any): void {
        if (!param.type) {
            throw createAppError(
                ...
                400,
                'Invalid payload for message type: template',
                { messageType: 'template', validationErrors: [{ field: 'parameter.type', message: 'Parameter type is required' }] }
            );
        }
        ...
        if (!validTypes.includes(param.type)) {
            throw createAppError(
                ...
                400,
                'Invalid payload for message type: template',
                { messageType: 'template', validationErrors: [{ field: 'parameter.type', message: `Invalid parameter type: ${param.type}` }] }
            );
        }
            ...
            case 'text':
                if (!param.text) {
                    throw createAppError(
                        ...
                        400,
                        'Invalid payload for message type: template',
                        { messageType: 'template', validationErrors: [{ field: 'parameter.text', message: 'Text parameter must have text field' }] }
                    );
                }
                break;
            ...
            case 'currency':
                if (!param.currency || !param.currency.code || !param.currency.amount_1000) {
                    throw createAppError(
                        ...
                        400,
                        'Invalid payload for message type: template',
                        { messageType: 'template', validationErrors: [{ field: 'parameter.currency', message: 'Currency parameter must have code and amount_1000' }] }
                    );
                }
                break;
            ...
            case 'date_time':
                if (!param.date_time || !param.date_time.fallback_value) {
                    throw createAppError(
                        ...
                        400,
                        'Invalid payload for message type: template',
                        { messageType: 'template', validationErrors: [{ field: 'parameter.date_time', message: 'DateTime parameter must have fallback_value' }] }
                    );
                }
                break;
            ...
            case 'video':
                if (!param[param.type] || !param[param.type].link) {
                    throw createAppError(
                        ...
                        400,
                        'Invalid payload for message type: template',
                        { messageType: 'template', validationErrors: [{ field: `parameter.${param.type}`, message: `${param.type} parameter must have link` }] }
                    );
                }
                break;
        }
