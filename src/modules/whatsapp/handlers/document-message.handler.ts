    ...
    validate(message: DocumentMessage, context: BuildContext): void {
        if (!message.link && !message.id) {
            throw createAppError(
                ...
                400,
                'Invalid payload for message type: document',
                { messageType: 'document', validationErrors: [{ field: 'link/id', message: 'Either link or id must be provided' }] }
            );
        }
        ...
        if (message.link && message.id) {
            throw createAppError(
                ...
                400,
                'Invalid payload for message type: document',
                { messageType: 'document', validationErrors: [{ field: 'link/id', message: 'Provide either link OR id, not both' }] }
            );
        }
        ...
        if (message.caption && message.caption.length > 1024) {
            throw createAppError(
                ...
                400,
                'Invalid payload for message type: document',
                { messageType: 'document', validationErrors: [{ field: 'caption', message: `Caption exceeds maximum length of 1024 characters` }] }
            );
        }
