...
| A | ✅ **done 2026-09-08** | Session + durable profile + the gate, `test:negotiation` **35/0**. `POST /internal/negotiation/{context,record}`. **The port is now IMPLEMENTED and REGISTERED** — `test:negotiation-lock` **41/0** (DB-free) + `verify:negotiation-lock` **15/0** (real Mongo, incl. the transaction rollback). ⚠ Read the notes: the resolver went unregistered for a day and every haggled add-to-cart 500'd, and `consumed_by_order_id` is left null by construction. |
