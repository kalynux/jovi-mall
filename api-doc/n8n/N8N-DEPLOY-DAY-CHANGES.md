...
| 2026-09-22 04:40:28 | mcp `c8929554` | **the handset test, after the backend deploy (`a430d64`)** — `UP-wi-mall-mcp`: + `checkout_review`, `checkout_place`, `checkout_payment_status`, `checkout_retry_payment` (the last two were mounted and never catalogued), − `payment_create_pay_link` (card-only, cards off), `orders_list_groups` / `inapp_open_orders` descriptions. 58 ops rendered `--existing` against `fb56fc33`, re-rendered after the deploy byte-identical; the diff showed the four new tools carrying the bearer credential (n8n's "configure manually" note is spurious). 58 → 61 tools; rollback `fb56fc33` | `test:bot-surface` 313/0 |
| 2026-09-22 04:43:33 | `05616e98` | core: `compose agent reply` drops a sentence the model copied out of a successful tool message (exec 1942); `AI Agent` systemMessage gains ## CHECKOUT, ## ORDERS, "never copy a tool's sentence", ## WHEN THE CUSTOMER TYPES INSTEAD OF TAPPING, and "an agreed price is normally already in the basket" — insertions only. Built on `63c59a2f`; both bodies byte-identical to the tested files (sha 2a8ad2b9… / aa343851…), only those two nodes changed, connections identical. Rollback `63c59a2f` | `test-chat-without-buttons.js` 47/0 · `build-checkout-prompt.js` 18/0 |
| 2026-09-22 04:47:01 | playbook v2 | **not n8n** — `seed:negotiation-playbook` on production `jovi_mall` over the tailnet (dry-run first): v2 `5088eed6…` active, v1 `b405ccb8…` superseded and kept. The bargainer never asks for an address or a phone, and a close offers checkout. The server caches it 60 s | `test:negotiation-playbook` 32/0 |
| 2026-09-22 06:18:33 | bargain `10d4f6fd` | **the second handset test** (core execs 2245–2317, bargain 2259/2266) — one message opened TWO haggles, the first for a product replayed from a 4-hour-old call in memory, and both answers were sent. `Bargain Memory` keyed per product (`wi-mall:bargain-chat:{channel}:{externalId}:{productId}`, the product as `pick variant` resolved it, so open and turn name one key), window 20 → 10; open mode reads `echo answered` first and a second open for the SAME message answers `alreadyAnswered` and sends nothing (+4 nodes, 40 → 44). Rollback `331e790e` | `build-handset-2.js` 49/0 |
| 2026-09-22 06:22:30 | `9ee8dbf0` | **the second handset test, core half** — `Chat Memory` 20 interactions / 3 days → **6 / 2 h** (owner's call: prompts were ~25 000 tokens of stale tool output, one carrying an old botToken); systemMessage: CHECKOUT reads a server-drawn confirmation when `checkout_review` returns `reply` and "a question is not a yes", new ## PAYMENT STATUS (waiting = patience, never a second request — exec 2317), one `open_negotiation` per message, act on the latest message; `compose agent reply` drops a model sentence that ends mid-way when a tool message or cards carry the turn (exec 2294: "Your order is 200 XAF,"). Rollback `05616e98` | `build-handset-2.js` 49/0 |
| 2026-09-22 13:44:33 | server `792c6f5` | **not n8n** — the owner's redeploy; both replicas booted at this second (their boot-time index-drift warnings, which also fired at every boot since 2026-09-21 04:07 and predate this change). The server now draws the chat checkout's confirmation and placement message, and a payment result returns to the chat the checkout came from (`IPaymentTransaction.originChat`) | CI + release image green |
| 2026-09-22 13:49:20 | mcp `fd7ccb78` | rendered `--existing` against live `c8929554` from `catalog.json` at `792c6f5`: only the four checkout tools' descriptions (the review and the place draw their own messages; a waiting payment = patience; no retry while waiting). 8 ops, 61 tools, diff = 4 `toolDescription`s + the sticky re-emitted byte-identical. Rollback `c8929554` | per-node sha identical |
| 2026-09-22 13:50:37 | `ba3096cf` | **owner's decision** — core's two brains swap slots: `OpenRouter Main` = `openai/gpt-5.6-luna`, `OpenRouter Fallback` = `qwen/qwen3.8-flash` (qwen truncated 3 of ~11 final answers in test 2; luna's fallback turns in test 1 were all complete). Only the two `model` values; the bargainer is untouched (qwen first). Rollback `9ee8dbf0` | diff = 2 params |
| 2026-09-22 15:14:43 | bargain `bd7930c9` | ⛔ **a regression of ours, live from 06:18 to here** (third handset test, core 2850 → bargain 2851, 14:06 UTC): `10d4f6fd` put the three answered-check nodes between `route mode` and `resolve variant`, whose body read `$json.productId`/`$json.variantId` — now `{ answeredEcho, alreadyAnswered }`. Every open sent `{}`, jovi-mall answered `NEGOTIATION_TOOL_SUBJECT_REQUIRED`, `pick variant` made that `product_not_found`, and **no haggle could start for 9 hours**. The body reads `$('Inbound')` now. One parameter. The harness also pins the general property: no node behind the answered check reads an Inbound field off `$json` (`build-handset-2.js` evaluated the three new nodes and never the one after them). Rollback `10d4f6fd` | `build-bargain-subject-fix.js` 11/0, 2 mutants; diff = 1 param |
| 2026-09-22 15:25:39 | `8942c69b` | **third handset test** (core 2819–2903): ⛔ an EMPTY agent answer is **not saved to the chat memory at all** — n8n skips the save, tool calls included (2884 "The biscuits!!": `ai.agent.memory.saves: 0`). Our prompt asked for exactly that ("write nothing") whenever a tool's own message carries the turn, so the turn that drew the order summary vanished and "yes please" was answered "Which item would you like me to add 2 of?" (2892); the turn that filled the basket vanished and "Place order please" was answered "already placed" (2827 → 2834). systemMessage: every "write nothing" → `[sent]`, plus one rule saying why; `compose agent reply` removes `[sent]` (and a bare "sent") before sending. Customer-visible output for every answer without the marker is byte-identical. Rollback `ba3096cf` | `build-memory-marker.js` 26/0, 2 mutants; diff = 2 params |
| 2026-09-22 17:43:41 | bargain `667a7f7f` | **administrators reset a customer's bot memory** (owner's decision; jovi-mall `6008fda`, wi-admin `d09e608`): `Inbound` gains an UNTYPED `memoryEpoch` input (a typed blank throws before the callee runs); `Bargain Memory` appends `:e<N>` when N > 0. Epoch 0 / absent / malformed = today's key byte for byte, so it was safe ahead of the backend. Rollback `bd7930c9` | `build-memory-epoch.js` 27/0; diff = 2 params |
| 2026-09-22 17:44:39 | `b59c40a2` | the core half: `Chat Memory` appends `:e<N>` from `sync identity` → `data.customer.memoryEpoch`; `open_negotiation` AND `hand to bargainer` pass `memoryEpoch` as a string, so a reset reaches the bargainer's memory on both roads in. jovi-mall never touches n8n's Redis — bumping the epoch makes the old memory unreachable and it lapses on its own TTL. Rollback `8942c69b` | `build-memory-epoch.js` 27/0; diff = 3 params |
| 2026-09-22 18:22:30 | `34fe8fe5` | **§ 8.2a** (asked for by the session building the screen half; INERT until its jovi-mall change deploys — no `pendingBargain`, the IF is false, every turn byte-identical): three inline nodes between `sync identity` and `detect command` write the SAME bargaining flag a Bargain TAP writes, from `data.pendingBargain`. ⛔ **One departure from the spec I was handed, and it is the morning's lesson again:** `detect command` read its sync body from `$input`, and a Redis node emits an API result rather than the item that entered it — the spec's wiring would have fed it `{success:true}` on every screen-press turn and lost `data.customer.botToken` downstream. It reads `$('sync identity')` by name now. Rollback `b59c40a2` | `build-screen-bargain-flag.js` 24/0, mutant = the live node fed a Redis result |
