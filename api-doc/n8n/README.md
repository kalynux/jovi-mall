...
| **[MCP-PARITY-PLAN.md](./MCP-PARITY-PLAN.md)** | ⭐ **The build plan for MCP.** Every customer action on `frontend/landing` mapped to a tool — 35 gaps, all of them thin wrappers over endpoints that already exist — in eleven steps, plus the seven rows deliberately left unbuilt and why | Adding any MCP tool, or planning the next batch |
...
| **[negotiation-tools.md](./negotiation-tools.md)** | **The built contract** for `/api/internal/negotiation/tools/*` — the bargaining sub-agent's five READ tools. The floor/ask vocabulary, why a budget bounds the **floor** and not the ask, and the three answers the platform deliberately cannot give (no ETA, no promotion, no compatibility verdict) | Wiring the bargaining sub-agent's tool nodes |
| **[typing-indicator.md](./typing-indicator.md)** | **The built "typing…" loop** that runs for the whole turn rather than Telegram's free 5 seconds. Read for the two platform windows and why the ping intervals are 4 s / 20 s, the WhatsApp blue-ticks side effect — and ⛔ the measured fact that **unpublishing `wi-mall-typing` does NOT stop it** | Changing either adapter, or switching the indicator off |
| **[bargaining-agent.md](./bargaining-agent.md)** | **The built n8n half** of the bargaining sub-agent (Stream F): the three workflows, why the playbook sits in the system position byte-identical, why the negotiation context arrives as a *tool result* rather than in the prompt, and how D-4 is enforced by a Redis echo instead of by the model's good behaviour. ⛔ Ends with the one gap that keeps the feature from working end-to-end | Touching any of the three workflows, or wondering why a minted lock is never spent |
| **[vectoriser/README.md](./vectoriser/README.md)** | The **product index** the shopping agent searches: the three paths `VectorisationService` already calls, the new 202-plus-callback contract, what the embedded text does and does not carry, and the two routes jovi-mall still has to grow. Its [`product_vectors.sql`](./vectoriser/product_vectors.sql) is the pgvector schema | Touching vectorisation, product search, or the bargaining agent |
